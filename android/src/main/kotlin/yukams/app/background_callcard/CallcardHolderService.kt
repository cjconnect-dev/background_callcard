...
import yukams.app.background_callcard.pluggables.Pluggable
...
class CallcardHolderService : MethodChannel.MethodCallHandler, LocationUpdateListener, Service(), BasicMessageChannel.MessageHandler<Any?> {
    companion object {
        ...
        @JvmStatic
        val ACTION_SHOW_OVERLAY_VIEW = "ACTION_SHOW_OVERLAY_VIEW"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when {
            ...
            ACTION_CLOSE_OVERLAY_VIEW == intent?.action -> {
                if (isServiceRunning) {
                    closeOverlayView(intent)
                }
            }
        }
    ...
    private fun showOverlayView(intent: Intent) {
        overlayCallCardView?.addOverlayViewAttach()
    }

    private fun closeOverlayView(intent: Intent) {
        overlayCallCardView?.removeOverlayViewDetach()
    }
